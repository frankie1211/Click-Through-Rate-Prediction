...
class LogisticRegression extends LinearModel {
  override def hyperParameterTuning(data: RDD[LabeledPoint],test:RDD[LabeledPoint], iteration: List[Int] = List(10, 100, 1000), threshold: List[Double]): List[(LogisticRegressionModel, (Double, Double), (Int, Double))] = {
